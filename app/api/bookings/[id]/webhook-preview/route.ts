...
export async function GET(
  ...
  // ---------- Auth (same pattern as /api/bookings/[id]) ----------
  ...
  if (bookingErr) {
    return NextResponse.json(
      { error: 'Database error', detail: bookingErr.message },
      { status: 500 }
    );
  }
  ...
  // ---------- reschedule_count ----------
  // Count answered_rescheduled call_attempts for ANY booking owned by this attendee,
  // since this attendee's most-recent signed_in_at (or epoch start).
  ...
  // ---------- event_type inference (first match wins) ----------
  ...
  if (bookingRow.attendance_status === 'no_show') {
    ...
    // Distinguish 'rescheduled' from 'declined' by checking for a downstream booking
