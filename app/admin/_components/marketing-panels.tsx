...
export async function MarketingPanels({ eventId }: { eventId: string | null }) {
  const [channels, insights, conversion] = await Promise.all([
    getChannelFunnels(eventId ? { eventId } : undefined),
    getSurveyInsights(eventId ? { eventId } : undefined),
    getConversionTotals(eventId),
  ]);
